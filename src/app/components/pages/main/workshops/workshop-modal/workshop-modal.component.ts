...
export class WorkshopModalComponent implements OnInit {
  ...
  // 👉 Convierte Date a string 'YYYY-MM-DD', acepta null
toDateString(date: Date | null): string {
  ...
  if (this.selectedPersonIds.length === 0) {
    return; // No envía si no hay personas seleccionadas
  }
  ...
  // ...existing code...
get kinshipTypes(): string[] {
  return Array.from(new Set(this.personList.map(p => p.typeKinship)));
}

get filteredPersonList() {
  if (!this.typeKinshipFilter) return this.personList;
  return this.personList.filter(p => p.typeKinship === this.typeKinshipFilter);
}

onKinshipFilterChange(): void {
  // Solo deja seleccionados los que están en la lista filtrada
  const filteredIds = this.filteredPersonList.map(p => p.idPerson);
  this.selectedPersonIds = this.selectedPersonIds.filter(id => filteredIds.includes(id));
}
allowOnlyLetters(event: any): void {
  const input = event.target as HTMLTextAreaElement;
  // Solo letras y espacios, elimina números y guiones
  input.value = input.value.replace(/[^a-zA-ZáéíóúÁÉÍÓÚñÑüÜ\s]/g, '');
  this.formData.description = input.value;
}

allowOnlyLettersName(event: any): void {
