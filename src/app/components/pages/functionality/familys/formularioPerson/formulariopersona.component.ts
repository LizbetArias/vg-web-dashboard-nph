...
export class FormulariopersonaComponent implements OnChanges {
  ...
  isCurrentPersonValid(): boolean {
    return (
      ...
      this.currentPerson.educationLevel?.trim() !== ''
    );
  }
  ...
  validateEducationLevel(): void {
    if (!this.currentPerson.educationLevel) {
      this.validationErrors['educationLevel'] = 'Debe seleccionar un grado de instrucción';
    } else {
      delete this.validationErrors['educationLevel'];
    }
  }
