...
export class IssueModalComponent implements OnInit{
  ...
  if (this.issueForm.id) {
    this.issueService.updateIssue(this.issueForm.id, this.issueForm).subscribe({
      next: (updatedIssue) => {
        ...
        this.saveIssueEvent.emit(updatedIssue); // 🔥 Notifica al padre para actualizar
        this.closeModal();
      },
      error: (err) => {
  ...
  this.issueService.createIssue(this.issueForm).subscribe({
    next: (savedIssue) => {
      ...
      this.saveIssueEvent.emit(savedIssue);  // 🔥 Notifica al padre para que actualice la lista
      this.closeModal();
    },
    error: (err: any) => {
